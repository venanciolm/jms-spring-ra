...
public class JMSConfigInboundArtemis {
	private static final Logger log = LoggerFactory.getLogger(JMSConfigInboundArtemis.class);

	@Bean("ConsumerJMSCF")
	public XAConnectionFactory getContainer( //
			@Value("${activemq.consumer.urlVM:vm://0}") String uri //
	) throws JMSException {
		// JmsConnectionFactory fisicalCF = new JmsConnectionFactory(user, password,
		// uri);
